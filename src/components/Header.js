...
import Logo from '../assets/sid_logo.svg'
...
const Header = ({lightMode, toggleMode}) => {
    return (
        <HeaderContainer initial={{opacity: 0}} animate={{opacity: 1}} transition={{duration: 1}}>
            ...
            <NavlinksContainer>
                {!lightMode && <ModeIcon initial={{opacity: 0}} animate={{opacity: 1}} transition={{duration: 2}} onClick={() => toggleMode(true)}><FontAwesomeIcon color="#FFF" icon={faSun} size="2x" /></ModeIcon> }
